...
public class ProjectsService(AppDbContext dbContext, OrganizationNodeService organizationNodeService)
{
    ...
    public async Task<ServiceResult<ProjectResponse>> GetByIdAsync(int id)
    {
        ...
        if (project == null) return ServiceResult<ProjectResponse>.NotFound("Project not found.");
    ...
    public async Task<ServiceResult<List<DepartmentResponse>>> GetProjectDepartmentsAsync(int id)
    {
        ...
        var departments = await dbContext.Departments.Where(d => d.ProjectId == id).Select(d => d.ToResponse()).ToListAsync();
    ...
    public async Task<ServiceResult<ProjectResponse>> CreateAsync(CreateProjectRequest request, int divisionId)
    {
        ...
        if (codeExists) return ServiceResult<ProjectResponse>.BadRequest("Project code already exists for the division.");
        ...
        {
            var leaderResult = await organizationNodeService.SetLeaderAsync(dbContext.Projects, projectEntity.Id, request.LeaderId.Value);

            if (!leaderResult.Success)
                return new ServiceResult<ProjectResponse> { Success = false, Error = leaderResult.Error, StatusCode = leaderResult.StatusCode };
    ...
    public async Task<ServiceResult> DeleteAsync(int id)
    {
        ...
        if (project == null) return ServiceResult.NotFound("Project not found.");
    ...
    public async Task<ServiceResult<ProjectResponse>> UpdateAsync(int projectId, UpdateOrganizationNodeRequest request)
    {
        ...
        var result = await organizationNodeService.UpdateAsync(dbContext.Projects, dbContext.Projects.Where(p => p.DivisionId == project.DivisionId), projectId, request);

        if (!result.Success)
            return new ServiceResult<ProjectResponse> { Success = false, Error = result.Error, StatusCode = result.StatusCode };
